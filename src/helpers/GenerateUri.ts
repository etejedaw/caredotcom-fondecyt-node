...
class GenerateUri {
	...
	getOfferLinks(): Array<UriData> {
		const links = Environment.OFFERS.map(offerName => {
			return Environment.LOCAL_AREAS.map(localArea =>
				this.#uriCombinations(offerName, localArea, DataType.OFFER)
			);
		});
		return links.flat();
	}
	...
	getJobsLinks(): Array<UriData> {
		const links = Environment.JOBS.map(jobName => {
			return Environment.LOCAL_AREAS.map(localArea =>
				this.#uriCombinations(jobName, localArea, DataType.JOB)
			);
		});
		return links.flat();
	}
