...
class ScrapeJobProvider {
	...
	extractDataV1(): JobProvider[] {
		...
		return $(
			".container.seo-listing .row .row.search-results .col-xs-12 .search-result"
		)
			.map((idx, elem) => {
	...
	extractDataV2(): JobProvider[] {
		const $ = cheerio.load(this.#html);
		return $(
			".col-lg-9.search-results .row .col-xs-12.results .group .search-result"
		)
			.map((_idx, elem) => {
				const title = $(elem).find(".profile-name .visible-sm").text().trim();
				const workSchedule = $(elem)
					.find(".col-xs-7.col-sm-3.text-block")
					.text()
					.trim();
				const priceRank = $(elem)
					.find(".col-xs-5.col-sm-2.text-block")
					.text()
					.trim();
				const data = { title, workSchedule, priceRank } as JobProvider;
				if (ObjectExtended.hasEmptyValues(data)) return;
	...
	extractDataV3(): JobProvider[] {
		const $ = cheerio.load(this.#html);
		return $(".row.search-res-section .col-xs-12.col-lg-9 .search-result")
			.map((_idx, elem) => {
				const title = $(elem).find(".job-title").text().trim();
				const workSchedule = $(elem)
					.find(".row.job-detail-info .col-xs-6.col-sm-6")
					.text()
					.trim();
				const priceRank = $(elem)
					.find(".row.job-detail-info .col-xs-5.col-sm-3")
					.text()
					.trim();
				const data = { title, workSchedule, priceRank } as JobProvider;
				if (ObjectExtended.hasEmptyValues(data)) return;
