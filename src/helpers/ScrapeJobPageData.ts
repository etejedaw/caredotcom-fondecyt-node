...
class ScrapeJobPageData {
	...
	extractDataV1(): JobPageData {
		const query =
			".container-fluid .heroImg .col-xs-12.text-center.intro .body-1.hero-subhead";
		return this.#execScrapeQuery(query);
	}

	extractDataV2(): JobPageData {
		const query =
			".body-content.visitor-seg .row .col-xs-12 .letterbox-expander .letterbox .sub-headline";
		return this.#execScrapeQuery(query);
	}
